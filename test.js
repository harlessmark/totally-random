...
var TotallyRandom = require("./dist/index.js");
...
describe("TotallyRandom", () => {
    describe("#array", () => {
        it("should throw an error when Array length is less than count with unique=true", () => {
            expect(() => random.array([1, 2, 3], count=4, unique=true)).to.throw("Array length must be larger than count if unique = true")
        })
    })
    describe("#between", () => {
    ...
    describe("#shuffle", () => {
        it("should throw an error when Array.length < 2", () => {
            expect(() => random.shuffle([1])).to.throw("arr must contain at least two elements")
        })
    })
    describe("#string", () => {
        it("should throw an error when length < 1", () => {
            expect(() => random.string(length=0.5)).to.throw("length must be at least 1")
        })
    })
    describe("#to", () => {
        it("should throw an error when count < 1", () => {
